...
#    FAexpi = find('*ra-decexp.fits', '.')
    ...
    FIerri = find('*noise_map_sig.fits', '.')
...
#    NAexpi = find('*ra-decexp.fits', '.')
    ...
    NIerri = find('*noise_map_sig.fits', '.')
...
#    NAexp = glob('*ra-decexp.fits')
    ...
    if len(Nsnr) == 1: 
        ...
        re_result = re.search('N/uvtN\.\d{2}/uvtC', str(history))
